...
function scheduleForOffset(dayOffset: number): string {
  const d = new Date(nextMonday());
  d.setDate(d.getDate() + Math.max(0, Math.min(6, dayOffset)));
  d.setHours(10, 0, 0, 0);
...
export default function WeeklyPackPage() {
  ...
  async function generate() {
    ...
    setGenError(null);
    try {
      const items = await generateWeeklyPack(brand.id, selectedPlatforms);
      setCards(
        items.map((item, i) => ({
          ...item,
          id: `${Date.now()}-${i}`,
          scheduledAt: scheduleForOffset(item.dayOffset),
          imageUrl: null,
          imageLoading: false,
          imageError: null,
        }))
    ...
    <div className="mx-auto max-w-5xl p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="font-display text-2xl font-bold text-ink">Haftalık İçerik Paketi</h1>
      ...
      {step === "submitted" ? (
        <div className="mt-8 flex flex-col items-center gap-4 rounded-2xl border border-line bg-surface p-10 text-center sm:p-12">
          ...
          <div>
            <h2 className="font-display text-xl font-bold text-ink">5 Günlük İçerik Paketi Onaya Gönderildi!</h2>
        ...
        <div className="mt-6 space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="font-display text-lg font-bold text-ink">Önümüzdeki Haftanın 5 Günlük İçerikleri</h2>
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-line/60 pt-3">
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) => updateCard(card.id, { scheduledAt: e.target.value })}
                      className="rounded-lg border border-line bg-bg px-2.5 py-1.5 font-body text-xs text-ink focus:border-accent focus:outline-none"
                    />
                  </div>
        ...
        <div className="mt-6 rounded-2xl border border-line bg-surface p-6 sm:p-8">
          ...
          <div className="mt-6 pt-6 border-t border-line">
            <button
              type="button"
              onClick={generate}
              disabled={selectedPlatforms.length === 0 || step === "generating"}
            ...
            >
              {step === "generating" ? (
                <>
                  <span className="h-4 w-4 rounded-full border-2 border-bg border-t-transparent animate-spin" />
                  <span>5 Günlük İçerik Paketi Üretiliyor…</span>
                </>
              ) : (
                <>
                  <span>✨ 5 Günlük İçerik Paketini AI ile Üret</span>
                </>
              )}
            </button>
          </div>
