...
export default function WooCommerceProductPicker({ onSelect, onClose }: Props) {
  const { t } = useLanguage();
  const wc = t.dashboard.woocommerce;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (ignore) return;
        if (!res.ok) throw new Error(data.error || wc.errorRetrieve);
        setProducts(data.products ?? []);
      } catch (err) {
        if (!ignore) setError(err instanceof Error ? err.message : wc.errorRetrieve);
      } finally {
      ...
      <div className="relative z-10 flex max-h-[80vh] w-full max-w-2xl flex-col rounded-2xl bg-white shadow-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="shrink-0 border-b border-slate-100 px-5 py-3">
          <input
            ...
            placeholder={wc.searchPlaceholder}
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {loading ? (
            <p className="text-center text-xs text-slate-400">{wc.loading}</p>
          ) : error ? (
            <p className="text-center text-xs text-red-600">{error}</p>
          ) : products.length === 0 ? (
            <p className="text-center text-xs text-slate-400">{wc.notFound}</p>
          ) : (
