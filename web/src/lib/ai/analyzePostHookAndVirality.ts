...
3. "alternative_hooks": Metnin konusunu çok daha çarpıcı kılan, kaydetme ve yorum tetikleyecek 2 adet alternatif açılış kancası (tek cümle).
...
5. SADECE ve SADECE aşağıdaki JSON formatında yanıt üret, başında veya sonunda markdown bloğu (\`\`\`) veya açıklama ekleme:
{
  ...
  const alternativeHooks = rawHooks.map(String).filter((h: string) => h.trim().length > 0).slice(0, 2);
